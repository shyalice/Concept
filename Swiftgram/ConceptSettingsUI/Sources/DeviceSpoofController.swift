...
private enum DeviceSpoofEntry: ItemListNodeEntry {
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .customDeviceModel(_, title, value):
            return ItemListSingleLineInputItem(presentationData: presentationData, systemStyle: .glass, title: NSAttributedString(string: title, textColor: presentationData.theme.list.itemPrimaryTextColor), text: value, placeholder: "iPhone 14 Pro", sectionId: self.section, textUpdated: { text in
                arguments.updateCustomDeviceModel(text)
            }, action: {})
        case let .customSystemVersion(_, title, value):
            return ItemListSingleLineInputItem(presentationData: presentationData, systemStyle: .glass, title: NSAttributedString(string: title, textColor: presentationData.theme.list.itemPrimaryTextColor), text: value, placeholder: "iOS 17.2", sectionId: self.section, textUpdated: { text in
...
private func deviceSpoofControllerEntries(presentationData: PresentationData, state: DeviceSpoofControllerState) -> [DeviceSpoofEntry] {
    ...
    entries.append(.enableHeader(theme, "DEVICE SPOOFING"))
    entries.append(.enableToggle(theme, "Enable Spoofing", state.isEnabled))
    entries.append(.enableInfo(theme, "Changes device info for Telegram servers. App restart required."))
    
    entries.append(.profilesHeader(theme, "SELECT DEVICE"))
    ...
    if state.selectedProfileId == 100 {
        entries.append(.customHeader(theme, "CUSTOM DEVICE"))
        entries.append(.customDeviceModel(theme, "Model: ", state.customDeviceModel))
        entries.append(.customSystemVersion(theme, "System: ", state.customSystemVersion))
        
        // Warning if fields are empty
        if state.customDeviceModel.isEmpty || state.customSystemVersion.isEmpty {
            entries.append(.customInfo(theme, "⚠️ Fill both fields. If empty, the real device is used."))
        } else {
            entries.append(.customInfo(theme, "Restart the app to apply changes."))
        }
    }
