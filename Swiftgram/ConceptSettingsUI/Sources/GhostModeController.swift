...
private func ghostModeControllerEntries(presentationData: PresentationData, state: GhostModeControllerState) -> [GhostModeEntry] {
    ...
    // Master section
    entries.append(.masterHeader(theme, "GHOST MODE"))
    entries.append(.masterToggle(theme, "Ghost Mode", state.isEnabled, activeCount, 5))
    entries.append(.masterInfo(theme, "When enabled, the selected privacy features will be active."))
    
    // Features section
    entries.append(.featuresHeader(theme, "FEATURES"))
    entries.append(.hideReadReceipts(theme, "Hide Read Receipts", state.hideReadReceipts))
    entries.append(.hideStoryViews(theme, "Hide Story Views", state.hideStoryViews))
    entries.append(.hideOnlineStatus(theme, "Hide Online Status", state.hideOnlineStatus))
    entries.append(.hideTypingIndicator(theme, "Hide Typing Indicator", state.hideTypingIndicator))
    entries.append(.forceOffline(theme, "Force Offline", state.forceOffline))
