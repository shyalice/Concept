...
private func miscControllerEntries(presentationData: PresentationData, state: MiscControllerState) -> [MiscEntry] {
    ...
    entries.append(.masterHeader(theme, "ADVANCED FEATURES"))
    entries.append(.masterToggle(theme, "Misc", state.isEnabled, activeCount, 5))
    entries.append(.masterInfo(theme, "When enabled, the selected bypass features will be active."))
    
    entries.append(.featuresHeader(theme, "FEATURES"))
    entries.append(.bypassCopyProtection(theme, "Bypass Copy Protection", state.bypassCopyProtection))
    entries.append(.disableViewOnceAutoDelete(theme, "Disable View Once Auto-Delete", state.disableViewOnceAutoDelete))
    entries.append(.bypassScreenshotProtection(theme, "Bypass Screenshot Protection", state.bypassScreenshotProtection))
    entries.append(.blockAds(theme, "Block Ads", state.blockAds))
    entries.append(.alwaysOnline(theme, "Always Online", state.alwaysOnline))
