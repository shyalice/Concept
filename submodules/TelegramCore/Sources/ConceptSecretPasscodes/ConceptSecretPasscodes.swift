...
extension SharedDataKeys {
    public static let conceptSecretPasscodes = applicationSpecificPreferencesKey(103)
}

public struct ConceptSecretChatId: Codable, Hashable {
...
public struct ConceptNavigateTo: Codable, Equatable {
    public let accountId: AccountRecordId
    public let peerId: PeerId?
...
public struct ConceptSecretPasscode: Codable, Equatable {
    public let passcode: String
    public let active: Bool
    public let timeout: Int32?
    public let accountIds: Set<AccountRecordId>
    public let secretChats: Set<ConceptSecretChatId>
    public let onRevealNavigateTo: ConceptNavigateTo?
    
    public init(passcode: String, active: Bool, timeout: Int32?, accountIds: Set<AccountRecordId>, secretChats: Set<ConceptSecretChatId>, onRevealNavigateTo: ConceptNavigateTo?) {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.secretChats = try container.decodeIfPresent(Set<ConceptSecretChatId>.self, forKey: "sc") ?? []
        self.onRevealNavigateTo = try container.decodeIfPresent(ConceptNavigateTo.self, forKey: "ornt")
    }
    ...
    public func withUpdated(active: Bool) -> ConceptSecretPasscode {
        return ConceptSecretPasscode(passcode: self.passcode, active: active, timeout: self.timeout, accountIds: self.accountIds, secretChats: self.secretChats, onRevealNavigateTo: self.onRevealNavigateTo)
    }
}

public struct ConceptSecretPasscodes: Codable, Equatable {
    public let secretPasscodes: [ConceptSecretPasscode]
    ...
    public static var defaultSettings: ConceptSecretPasscodes {
        return ConceptSecretPasscodes(secretPasscodes: [], dbCoveringAccounts: [:], cacheCoveringAccounts: [:])
    }
    
    public init(secretPasscodes: [ConceptSecretPasscode], dbCoveringAccounts: [AccountRecordId : AccountRecordId], cacheCoveringAccounts: [AccountRecordId : AccountRecordId]) {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.secretPasscodes = try container.decode([ConceptSecretPasscode].self, forKey: "spc")
    ...
    public init(_ entry: PreferencesEntry?) {
        self = entry?.get(ConceptSecretPasscodes.self) ?? .defaultSettings
    }
    
    public init(_ transaction: AccountManagerModifier<TelegramAccountManagerTypes>) {
        let entry = transaction.getSharedData(SharedDataKeys.conceptSecretPasscodes)
        self.init(entry)
    }
    ...
    public func withCheckedTimeoutUsingLockStateFile(rootPath: String) -> ConceptSecretPasscodes {
        if let data = try? Data(contentsOf: URL(fileURLWithPath: appLockStatePath(rootPath: rootPath))), let state = try? JSONDecoder().decode(LockState.self, from: data) {
            return ConceptSecretPasscodes(secretPasscodes: self.secretPasscodes.map { sp in
...
public func updateConceptSecretPasscodes(_ accountManager: AccountManager<TelegramAccountManagerTypes>, _ f: @escaping (ConceptSecretPasscodes) -> ConceptSecretPasscodes) -> Signal<Void, NoError> {
    return accountManager.transaction { transaction -> Void in
        transaction.updateSharedData(SharedDataKeys.conceptSecretPasscodes, { current in
            let updated = f(ConceptSecretPasscodes(current))
